...
def run(save_dir: str):
    ...
    table_data = []
    table_data.append(
        [1, "The second column is longer."]
    )  # rows can have different length
    table_data.append(
        [2, "Table entries don't have to be strings,"]
    ...
    table_style.font_style = PPTXFontStyle().set(
        italic=True, name="Arial", color_rgb=(100, 200, 30)
    )
    # todo: cell-style
