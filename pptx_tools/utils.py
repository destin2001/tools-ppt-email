...
class TemporaryPPTXFile:
    ...
    def __init__(
        self, mode="w+b", suffix=".pptx", dir=None, raise_on_delete_error=True
    ):
        if not dir:
...
class _USE_DEFAULT:  # using a class allows typing
    def __str__(self):
        return (
            "This is a default value, used to express that a value should become default, which is indicated with "
...
def change_paragraph_text_to(paragraph, text):
    ...
    :return:
    """
    from pptx_tools.font_style import (
        PPTXFontStyle,
    )  # local import to prevent circle import error
...
def save_pptx_as_png(
    save_folder: Union[str, "LocalPath"],
    pptx_filename: str,
    ...
    pres.close()
    if (
        powerpoint.Presentations.Count == 0
    ):  # only close, when no other Presentations are open!
        powerpoint.quit()
    return True


def save_pptx_as_pdf(
    pdf_filename: Union[str, "LocalPath"], pptx_filename, overwrite: bool = False
) -> bool:
    """
...
def save_as_pdf(
    prs: pptx.presentation.Presentation, filename: str, overwrite: bool = False
) -> bool:
    """
...
def save_as_png(
    prs: pptx.presentation.Presentation, save_folder: str, overwrite: bool = False
) -> bool:
    """
    Save presentation as PDF.
    Requires to save a temporary *.pptx first.
